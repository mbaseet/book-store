...
export const ordersTable = sqliteTable(
  'orders',
  {
    ...
    paymentPlan: text('payment_plan').notNull().default('full_upfront'),
    paymentMethod: text('payment_method').notNull(),
    // Kept separate from the fulfillment lifecycle so a verified deposit is
    // never mistaken for a fully collected order.
    paymentStatus: text('payment_status').notNull().default('payment_submitted'),
    ...
    instapayDiscountAmount: integer('instapay_discount_amount').notNull().default(0),
    ...
    amountDueNow: integer('amount_due_now').notNull().default(0),
